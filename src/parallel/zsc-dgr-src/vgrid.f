...
! Local variables
 ...
 INTEGER :: MPI_ERR, xeast, xwest, ynorth, ysouth
...
! Assign ghost layers to momentum mesh
 xlg = xl - 1
 xug = xu + 1
 ylg = yl - 1
 yug = yu + 1

! Set computational domain limits for order parameter mesh
 xmin_f = xmin
 ymin_f = ymin
 xmax_f = 2*xmax - 1
 ymax_f = 2*ymax - 1

! Find offset for order parameter mesh
 xOffset = 0
 yOffset = 0
 IF ( mpi_coords(1) > 0 ) xOffset = 1
 IF ( mpi_coords(2) > 0 ) yOffset = 1

! Limits of order parameter mesh in this processor
 xl_f = 2*xl - 1 - xOffset
 xu_f = 2*xu - 1
 yl_f = 2*yl - 1 - yOffset
 yu_f = 2*yu - 1

! Assign ghost layers to order parameter mesh
 xlg_f = xl_f - 1
 xug_f = xu_f + 1
 ylg_f = yl_f - 1
 yug_f = yu_f + 1
...
! Ranks of the diagonal neighbours of this processor
 mpi_coords(1) = xeast
 mpi_coords(2) = ynorth
